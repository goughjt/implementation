...
/////////////////////////////////////////////////////////////////////////////
bool LoadNVM(const char* filename, vector<CameraT>& camera_data, vector<Point3D>& point_data,
              vector<Point2D>& measurements, vector<int>& ptidx, vector<int>& camidx,
              vector<string>& names, vector<int>& ptc, ofstream &os, int pointScale, int pointSize, vector<float>& averageCoords)
{
    ...
    for(int i=0; i<npoint; i++){
    	float f[3];
    	point_data[i].GetPoint(f);
    	fx+=f[0];
    	fy+=f[1];
    	fz+=f[2];
    }
    averageCoords.push_back(fx/npoint);
    averageCoords.push_back(fy/npoint);
    averageCoords.push_back(fz/npoint);
    for(int i=0; i<npoint; i++){
    	float f[3];
    	point_data[i].GetPoint(f);
    	
    	os << "var material = new THREE.SpriteCanvasMaterial( { \n color: 0x" << std::hex << ((1 << 24) + (ptc[3*i+0] << 16) + (ptc[3*i+1] << 8) + ptc[3*i+2]) << std::dec << ", \n program: program \n	} ); \n particle = new THREE.Sprite( material ); \n particle.position.x = "<< pointScale*(f[0]-averageCoords[0])*(1) << " \n particle.position.y = " << pointScale*(f[1]-averageCoords[1]) << " \n particle.position.z = " << pointScale*(f[2]-averageCoords[2])*(1) << " \n particle.scale.x = particle.scale.y = " << pointSize << "; \n group.add( particle ); \n ";
              ...
              vector<string>& names, vector<int>& ptc)
{
    ...
    //if(strstr(name, ".nvm"))return LoadNVM(name, camera_data, point_data, measurements, ptidx, camidx, names, ptc, os);
